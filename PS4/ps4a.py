...
def getWordScore(word, n):
    ...
    for i in range(len(word)):
        t1 += SCRABBLE_LETTER_VALUES.get(word[i],0)
    t2 = t1 * len(word)
    return wordScore + t2
...
def updateHand(hand, word):
    ...
    uHand = hand.copy()
    for i in range(len(word)):
        uHand[word[i]] = uHand.get(word[i],0) - 1
    return uHand
...
def isValidWord(word, hand, wordList):
    ...
    if (word in wordList):
        dictHand = hand.copy()
        for i in range(len(word)):
            if word[i] in dictHand.keys():
                dictHand[word[i]] = dictHand.get(word[i],0) - 1
            else:
                return False
        for k,v in dictHand.items():
            if v < 0:
                return False
        return True
    else:
